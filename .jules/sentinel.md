...
**Vulnerability:** The admin authentication middleware `middleware/auth-admin.js` verified tokens using a hardcoded secret ("ASSIGNMENT3$"), allowing an attacker to forge admin tokens if they knew this string.
**Learning:** Even if the token generation (login) uses a secure secret, if the verification middleware uses a different or hardcoded one, the system is vulnerable. Consistency in secret management is key.
**Prevention:** Use a centralized configuration or utility function (like `getJwtSecret()`) for accessing secrets across the entire application to ensure consistency and security.
