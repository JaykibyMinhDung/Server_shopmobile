...
## 2024-05-23 - Admin Authorization Bypass via Incorrect Middleware
**Vulnerability:** Admin routes were protected by user authentication middleware, allowing any logged-in user (even non-admins) to access sensitive admin endpoints.
**Learning:** The project had two middleware files (`auth.js` and `auth-admin.js`), but the admin routes imported the wrong one. Additionally, the unused `auth-admin.js` contained a hardcoded secret ("ASSIGNMENT3$") that didn't match the dynamic secret used for signing tokens, meaning it would have failed even if used.
**Prevention:**
1. Use distinct middleware for distinct roles (User vs Admin).
2. Ensure secrets are consistent across signing and verification (use a single `getJwtSecret()` utility).
3. Verify role claims (`role === 'admin'`) inside the middleware, not just token validity.
